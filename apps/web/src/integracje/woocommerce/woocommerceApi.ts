...
const SCIEZKA_BAZOWA = "/integracje/woocommerce";

function rzucJesliBrakEndpointu(blad: unknown, nazwaFunkcji: string, endpoint: string): never {
  if (czyBladApi(blad) && blad.status === 404) {
    throw new BladNiezaimplementowane(`WooCommerce.${nazwaFunkcji}`, endpoint, blad);
  }
  throw blad;
}

export const woocommerceApi: KontraktIntegracji = {
  idIntegracji: "woocommerce",

  // odpowiada za pobranie statusu polaczenia WooCommerce
  ...
  // odpowiada za inicjalizacje autoryzacji WooCommerce
  ...
  // odpowiada za uruchomienie synchronizacji danych WooCommerce
  ...
  // odpowiada za odlaczenie konta WooCommerce
